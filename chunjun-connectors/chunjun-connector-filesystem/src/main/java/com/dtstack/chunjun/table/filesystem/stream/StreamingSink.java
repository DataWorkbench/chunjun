package com.dtstack.chunjun.table.filesystem.stream;
...
import com.dtstack.chunjun.streaming.api.functions.sink.filesystem.StreamingFileSink;
import com.dtstack.chunjun.table.filesystem.FileSystemFactory;
import com.dtstack.chunjun.table.filesystem.stream.compact.CompactFileWriter;
...
public class StreamingSink {
    ...
    public static <T> DataStream<PartitionCommitInfo> writer(
        ...
        StreamingFileWriter<T> fileWriter =
                new StreamingFileWriter<T>(bucketCheckInterval, bucketsBuilder);
        return inputStream
                .transform(
    ...
    public static <T> DataStream<PartitionCommitInfo> compactionWriter(
        ...
        SupplierWithException<FileSystem, IOException> fsSupplier =
                (SupplierWithException<FileSystem, IOException> & Serializable)
                        () -> fsFactory.create(path.toUri(), configuration);
        ...
        CompactOperator<T> compacter = new CompactOperator<>(fsSupplier, readFactory, null);
